...
const sections = [
  ...
  ["Staff roles and permissions", "A visible title such as Host or Moderator is separate from permissions. Owners can use role presets, customize individual capabilities, suspend access, set expirations, copy permissions, and protect high-risk Admin or Owner actions."],
  ["Discord webhooks", "Server staff with webhook permission can connect multiple Discord webhooks. Each destination can have its own label, sender, avatar, notification types, enabled state, test action, and encrypted webhook URL."],
  ["Teams", "Team profiles can manage rosters, recruiting, applications, invitations, affiliations with approved server profiles, and a main game. Roblox profiles can use a game name, Place ID, or game URL where supported."],
  ...
  ["Notifications", "Notifications show invitations, role changes, moderation updates, event activity, and other account-specific information. Read notifications can be deleted individually or cleared together."],
  ...
  ["Reports and moderation", "Public profiles can be reported. Authorized staff can review reports and take moderation actions only when their assigned permissions allow it."],
] as const;
  ...
  return (
    <main className="landing-shell legal-shell help-shell">
      ...
      <section className="help-quick-grid">
        ...
        <a className="card" href="#communities"><span className="card-kicker">Communities</span><h2>Manage servers and teams</h2><p>Build profiles, permissions, webhooks, rosters, audit trails, and moderation workflows.</p></a>
      </section>
      ...
      <section className="panel legal-copy section-stack" id="players">
        ...
        {sections.map(([title, description], index) => (
          <article className="help-step" id={index === 6 ? "hosts" : index === 8 ? "communities" : undefined} key={title}>
      ...
      <section className="panel section-stack">
        ...
        <div className="button-row"><Link className="button" href="/dashboard/profile">Game identities</Link><Link className="button button-secondary" href="/dashboard/events">Events</Link><Link className="button button-secondary" href="/dashboard/servers">Servers</Link><Link className="button button-secondary" href="/dashboard/access">Access Center</Link><Link className="button button-secondary" href="/dashboard/tools">Tools</Link></div>
      </section>
      <PublicFooter />
    </main>
  );
