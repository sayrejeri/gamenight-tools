...
import { TeamRosterControls } from "@/components/team-roster-controls";
  ...
  return (
    <main className="public-shell section-stack">
      ...
      {canManage ? <>
        ...
        <section className="panel section-stack"><div className="section-header"><div><h2>Manage roster</h2><p>Change active roles or remove members. The team owner cannot be removed or demoted from here.</p></div></div><div className="request-list">{members.map((member) => <article className="list-card roster-management-card" key={member.user_id}><span className="list-icon">{member.display_name.slice(0, 2)}</span><div><strong>{member.display_name}</strong><span>{member.role.toLowerCase()}</span></div><TeamRosterControls teamId={team.id} userId={member.user_id} currentRole={member.role} actorRole={viewerMembership.role} /></article>)}</div></section>
