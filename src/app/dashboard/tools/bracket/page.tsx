...
export default async function BracketToolPage({ searchParams }: { searchParams: Promise<{ eventId?: string }> }) {
  ...
  if (eventId) {
    ...
    if (!event) {
      ...
      const allowed = event.primary_host_id === session.userId || canManageCodes(role) || Boolean(cohost[0]);

      if (!allowed) {
        accessError = "You need bracket-manager permission for this event.";
      } else {
        ...
        const brackets = await query<(RowDataPacket & { settings_json: string | null })[]>(
          `SELECT settings_json FROM brackets WHERE event_id = ? LIMIT 1`,
          [eventId],
        );
        if (brackets[0]?.settings_json) {
          try {
            initialDraft = JSON.parse(brackets[0].settings_json);
          } catch {
            initialDraft = null;
          }
        }
