...
type NotificationRow = RowDataPacket & {
  id: string; title: string; message: string; category: string | null; action_url: string | null;
  is_read: number; created_at: Date; workspace_name: string | null;
};
...
function formatNotification(item: NotificationRow) {
  ...
  if (item.title === "Server access updated") {
    ...
    return { ...item, title: `${item.workspace_name} access updated`, message: role === "updated" ? `Your access for ${item.workspace_name} was updated.` : `Your role for ${item.workspace_name} is now ${role}.` };
  }
  return item;
}
...
export default async function NotificationsPage() {
  ...
  const notifications = await query<NotificationRow[]>(
    `SELECT n.id, n.title, n.message, n.category, n.action_url, n.is_read, n.created_at, w.name AS workspace_name
     FROM notifications n
     LEFT JOIN workspaces w ON n.category = 'SERVERS' AND n.action_url LIKE '/dashboard/workspaces/%' AND w.id = SUBSTRING_INDEX(n.action_url, '/', -1)
