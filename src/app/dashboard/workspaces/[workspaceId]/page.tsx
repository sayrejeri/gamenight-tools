...
export default async function WorkspacePage({ params }: { params: Promise<{ workspaceId: string }> }) {
  ...
  const [role, platformRole] = await Promise.all([
  ...
  const workspaces = await query<WorkspaceRow[]>(
    `SELECT w.id, w.discord_guild_id, w.name, w.icon_url, w.banner_url, w.description,
            ...
            (SELECT wm.role FROM workspace_members wm
             WHERE wm.workspace_id = w.id AND wm.user_id = ? AND wm.status = 'ACTIVE' LIMIT 1) AS member_role,
  ...
  const platformAccessLabel = isPlatformOwner(session.discordId)
    ? "PLATFORM OWNER"
    : platformRole === "OWNER" || platformRole === "ADMIN"
      ? `PLATFORM ${platformRole}`
      : null;
  const displayedRole = platformAccessLabel
    ? `${platformAccessLabel}${workspace.member_role ? ` · SERVER ${workspace.member_role}` : ""}`
    : workspace.member_role ?? role ?? "DISCORD SERVER MEMBER";
    ...
    <div className="section-stack">
      <section className="workspace-hero workspace-hero-banner" style={workspace.banner_url ? { backgroundImage: `linear-gradient(90deg, rgba(9,11,18,.95), rgba(9,11,18,.58)), url(${workspace.banner_url})` } : undefined}>
        <div className="organization-hero-main">
          ...
          <div>
            <span className="eyebrow">{displayedRole}</span>
            <h1>{workspace.name}</h1>
            <p>{workspace.description ?? "This server has not added a description yet."}</p>
            <div className="button-row">
              {workspace.main_game_category ? <span className="badge">Main game: {workspace.main_game_category}</span> : null}
              {workspace.verification_level ? <span className="badge">Verified: {workspace.verification_level.replaceAll("_", " ")}</span> : null}
              <span className="badge">Discord bot: optional</span>
              <span className="badge">Discord webhooks: supported</span>
              {workspace.chat_enabled ? <span className="badge">Website chat: planned</span> : null}
              {canEditProfile ? <span className="badge">You can manage this server</span> : null}
            </div>
          </div>
        </div>
      ...
      {canEditProfile && role ? <section className="panel section-stack"><div className="section-header"><div><h2>Server access management</h2><p>This private section is visible only to this server's owners/admins and authorized platform owners/admins.</p></div><span className="badge dashboard-count">Private</span></div><WorkspaceMemberManager workspaceId={workspaceId} actorRole={role} members={members.map((member) => ({ userId: member.user_id, displayName: member.display_name, siteUsername: member.site_username, discordId: member.discord_id, role: member.role, status: member.status, avatarUrl: member.avatar_url }))} ownerClaims={ownerClaims.map((claim) => ({ discordId: claim.discord_id, createdAt: new Date(claim.created_at).toISOString(), activeUserId: claim.active_user_id }))} /></section> : null}
