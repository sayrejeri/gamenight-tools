...
export default async function WorkspacePage({ params }: { params: Promise<{ workspaceId: string }> }) {
  ...
  const [events, games, templates, webhooks, members, ownerClaims] = await Promise.all([
    ...
    canManageMembers ? query<MemberRow[]>(
      `SELECT wm.user_id, wm.role, wm.display_label, wm.permissions_json, wm.status, wm.expires_at, wm.notes,
              u.discord_id, u.site_username, COALESCE(u.global_name, u.username) AS display_name,
  ...
  return (
    <div className="section-stack">
      ...
      {canHostEvents ? <section className="panel section-stack"><div className="section-header"><div><h2>Create an event</h2><p>Build a draft, preview it, then publish signups or submit it for approval.</p></div></div><CreateEventForm workspaceId={workspaceId} defaultTimezone={workspace.timezone} templates={templateOptions} workspaceGames={games} /></section> : null}
