...
export default async function StaffDashboardPage() {
  ...
  const [requests, reports, staff, audit] = await Promise.all([
    ...
    query<AuditRow[]>(
      `SELECT al.id, al.action_name, al.target_type, al.target_id, al.created_at,
       ...
       WHERE al.action_name LIKE 'profile_request.%' OR al.action_name LIKE 'platform_staff.%' OR al.action_name LIKE 'report.%'
  ...
  return (
    <div className="section-stack">
      <section className="page-heading"><div><span className="eyebrow">Private platform operations</span><h1>Staff dashboard</h1><p>Review organization profiles, handle reports, manage platform staff, and inspect the moderation audit trail.</p></div><span className="badge">{role}</span></section>
