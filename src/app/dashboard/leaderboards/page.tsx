    ...
    <div className="section-stack competitive-leaderboard-page">
      ...
      {mode === "players" ? (
        playerRows.length ? <section className="panel leaderboard-table-shell"><div className="leaderboard-table"><div className="leaderboard-row leaderboard-head"><span>Rank</span><span>Player</span><span>Record</span><span>Win rate</span><span>Titles</span><span>Best streak</span><span>Events</span></div>{playerRows.map((row, index) => {
          ...
          return <Link className={`leaderboard-row ${index < 3 ? "leaderboard-podium" : ""}`} href={`/users/${row.siteUsername}/competitive`} key={row.userId}><span className="leaderboard-rank">{index === 0 ? "🥇" : index === 1 ? "🥈" : index === 2 ? "🥉" : `#${index + 1}`}</span><span className="leaderboard-identity">{avatar ? <img src={avatar} alt="" /> : <span className="leaderboard-avatar-fallback">{row.displayName.slice(0, 1).toUpperCase()}</span>}<span><strong>{row.displayName}</strong><small>@{row.siteUsername}</small></span></span><span data-label="Record"><strong>{row.wins}–{row.losses}</strong><small>{row.matches} matches</small></span><span data-label="Win rate"><strong>{row.winRate}%</strong></span><span data-label="Titles"><strong>{row.championships}</strong></span><span data-label="Best streak"><strong>{row.bestWinStreak ? `W${row.bestWinStreak}` : "—"}</strong><small>{row.currentStreakType ? `Now ${row.currentStreakType}${row.currentStreak}` : "No streak"}</small></span><span data-label="Events"><strong>{row.eventsPlayed}</strong></span></Link>;
        })}</div></section> : <div className="empty-state">No player match results exist for these filters yet.</div>
      ) : (
        teamRows.length ? <section className="panel leaderboard-table-shell"><div className="leaderboard-table"><div className="leaderboard-row leaderboard-head"><span>Rank</span><span>Team</span><span>Record</span><span>Win rate</span><span>Titles</span><span>Best streak</span><span>Events</span></div>{teamRows.map((row, index) => <Link className={`leaderboard-row ${index < 3 ? "leaderboard-podium" : ""}`} href={`/teams/${row.slug}`} key={row.teamId}><span className="leaderboard-rank">{index === 0 ? "🥇" : index === 1 ? "🥈" : index === 2 ? "🥉" : `#${index + 1}`}</span><span className="leaderboard-identity">{row.logoUrl ? <img src={row.logoUrl} alt="" /> : <span className="leaderboard-avatar-fallback">{(row.tag ?? row.name).slice(0, 2).toUpperCase()}</span>}<span><strong>{row.name}</strong><small>{row.tag ? `[${row.tag}]` : "Approved team"}</small></span></span><span data-label="Record"><strong>{row.wins}–{row.losses}</strong><small>{row.matches} matches</small></span><span data-label="Win rate"><strong>{row.winRate}%</strong></span><span data-label="Titles"><strong>{row.championships}</strong></span><span data-label="Best streak"><strong>{row.bestWinStreak ? `W${row.bestWinStreak}` : "—"}</strong><small>{row.currentStreakType ? `Now ${row.currentStreakType}${row.currentStreak}` : "No streak"}</small></span><span data-label="Events"><strong>{row.eventsPlayed}</strong></span></Link>)}</div></section> : <div className="empty-state">No team match results exist for these filters yet.</div>
      )}
