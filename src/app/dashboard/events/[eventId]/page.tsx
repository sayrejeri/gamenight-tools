...
type EventRow = RowDataPacket & {
  ...
  bracket_enabled: number; bracket_format: string | null; bracket_entry_mode: "PLAYER" | "TEAM"; bracket_seeding_mode: string | null;
  bracket_auto_generate: number; bracket_require_check_in: number; bracket_group_count: number; bracket_advancers_per_group: number;
...
type ShareRow = RowDataPacket & { token: string; is_enabled: number; expires_at: Date | null };
type CountRow = RowDataPacket & { total: number };

function iso(value: Date | null): string | null { return value ? new Date(value).toISOString() : null; }
function formatLabel(value: string | null): string { return value ? value.replaceAll("_", " ").toLowerCase().replace(/\b\w/g, (letter) => letter.toUpperCase()) : "Not selected"; }
function competitionDescription(format: string | null): string {
  if (format === "DOUBLE_ELIMINATION") return "Winners bracket, losers bracket, and an automatic grand-final reset when required.";
  if (format === "ROUND_ROBIN") return "Every entrant plays every other entrant, with live standings and configurable tiebreaks.";
  if (format === "GROUPS_PLAYOFFS") return "Round-robin groups feed automatically seeded playoff matches once group standings are final.";
  if (format === "THREE_PLAYER") return "The custom A/B/C no-rematch advancement format with automatic rule resolution.";
  return "Single elimination with automatic byes and winner advancement.";
}
...
export default async function EventPage({ params }: { params: Promise<{ eventId: string }> }) {
  ...
  const events = await query<EventRow[]>(
    `SELECT e.id, e.workspace_id, w.name AS workspace_name, e.name, e.description, e.game_name,
            ...
            e.max_participants, e.cancellation_reason, e.cancelled_at, e.timezone, CAST(e.primary_host_id AS CHAR) AS primary_host_id,
  ...
  const canOperateMatches = isPrimaryHost || eventPermissions.includes("MANAGE_BRACKETS") || ["FULL", "BRACKET", "SCOREKEEPER"].includes(cohostLevel);
  const canManageShare = canManageEvent || canManageBracket;
  ...
  const [hostConnections, participants, cohosts, userConnections, bracketRows, teamCountRows, shareRows] = await Promise.all([
    ...
    event.bracket_entry_mode === "TEAM" ? Promise.resolve([] as ParticipantRow[]) : query<ParticipantRow[]>(
    ...
    event.bracket_entry_mode === "TEAM" ? query<CountRow[]>(`SELECT COUNT(*) AS total FROM event_team_entries WHERE event_id = ? AND status = 'REGISTERED'`, [eventId]) : Promise.resolve([] as CountRow[]),
    canManageShare && event.bracket_enabled ? query<ShareRow[]>(`SELECT token, is_enabled, expires_at FROM event_public_share_links WHERE event_id = ? LIMIT 1`, [eventId]) : Promise.resolve([] as ShareRow[]),
  ]);
  ...
  const teamCount = Number(teamCountRows[0]?.total ?? 0);
  const share = shareRows[0] ?? null;
  const appUrl = (process.env.APP_URL ?? "").replace(/\/$/, "");
  const initialShare = share ? {
    enabled: Boolean(share.is_enabled),
    url: appUrl ? `${appUrl}/spectate/${share.token}` : `/spectate/${share.token}`,
  ...
  return (
    <div className="section-stack">
      ...
      {event.bracket_entry_mode === "TEAM" ? <section className="panel section-stack"><div className="section-header"><div><h2>Your team entry</h2><p>Team owners, managers, and captains can register an approved team. The active competitive roster is snapshotted when the team enters.</p></div><span className="badge">{teamCount} registered</span></div><div className="button-row"><Link className="button" href={`/dashboard/events/${eventId}/teams`}>View / register teams</Link>{canManageParticipants ? <Link className="button button-secondary" href={`/dashboard/events/${eventId}/teams`}>Manage team field</Link> : null}</div></section> : <section className="panel section-stack"><div><h2>Your event spot</h2><p className="muted">{canManageEvent ? "You can manage this event and still participate in it." : event.signup_mode === "APPROVAL" ? "Complete your signup, then wait for a host to approve your spot." : "Sign up with the game identity required by the host, then check in when it opens."}</p></div><EventSignupControls eventId={eventId} eventStatus={event.status} participantStatus={participant?.status ?? null} participantSignupCompleted={Boolean(participant?.signup_completed_at)} checkedIn={Boolean(participant?.checked_in_at)} joinCodeRequired={Boolean(event.join_code_required)} signupMode={event.signup_mode} requiredConnectionType={event.required_connection_type} connections={userConnections} /></section>}

      {event.bracket_enabled ? <section className="panel section-stack"><div className="section-header"><div><h2>Tournament competition</h2><p className="muted">{formatLabel(event.bracket_format)} · {event.bracket_entry_mode === "TEAM" ? "teams" : "players"} · {event.bracket_seeding_mode?.toLowerCase()} placement{event.bracket_require_check_in ? " · checked-in participants only" : ""}</p></div>{bracket ? <span className="badge">{bracket.status}</span> : null}</div><div className="workspace-card"><span className="card-kicker">{event.bracket_auto_generate ? "Automatic after signups" : "Host controlled"}</span><h3>{formatLabel(event.bracket_format)}</h3><p>{competitionDescription(event.bracket_format)} Match Center handles ready checks, scheduling, reports, disputes, forfeits, and audited result advancement.</p>{event.bracket_format === "GROUPS_PLAYOFFS" ? <span className="field-help">{event.bracket_group_count} groups · top {event.bracket_advancers_per_group} advance · {formatLabel(event.bracket_tiebreak_mode)} tiebreak</span> : null}</div><div className="button-row">{event.bracket_entry_mode === "TEAM" ? <Link className="button button-secondary" href={`/dashboard/events/${eventId}/teams`}>Tournament teams</Link> : null}{bracket?.settings_json && (canManageBracket || ["LIVE", "COMPLETED"].includes(bracket.status)) ? <Link className="button button-secondary" href={`/dashboard/events/${eventId}/bracket`}>View competition</Link> : null}{bracket && (canOperateMatches || ["LIVE", "COMPLETED"].includes(bracket.status)) ? <Link className="button button-secondary" href={`/dashboard/events/${eventId}/matches`}>Open Match Center</Link> : null}{canManageBracket ? <Link className="button" href={`/dashboard/tools/bracket?eventId=${eventId}`}>Competition manager</Link> : null}</div>{canManageShare ? <EventShareLinkControls eventId={eventId} initialShare={initialShare} /> : null}</section> : null}

      {event.bracket_entry_mode === "PLAYER" ? <section className="panel section-stack"><div className="section-header"><div><h2>Participants</h2><p>{canManageParticipants ? "Pending, approved, and waitlisted players are visible to event staff. Private notes stay on the management screen." : "Game identity first, Discord identity underneath."}</p></div><div className="button-row"><span className="badge">{participants.length} listed</span>{canManageParticipants ? <Link className="button button-secondary" href={`/dashboard/events/${eventId}/participants`}>Manage participants</Link> : null}</div></div>{participants.length ? <div className="participant-card-grid">{participants.map((item) => { const gameName = item.game_identity_value ?? item.global_name ?? item.username; const gameProfile = item.game_identity_type ? buildConnectionProfileUrl(item.game_identity_type, item.connection_external_id, item.game_identity_value ?? item.username, item.connection_profile_url) : null; const avatar = item.connection_avatar_url ?? getDiscordAvatarUrl(item.discord_id, item.avatar_hash); return <article className="participant-card" key={item.user_id}>{avatar ? (gameProfile ? <a href={gameProfile} target="_blank" rel="noreferrer"><img className="identity-avatar" src={avatar} alt="" /></a> : <img className="identity-avatar" src={avatar} alt="" />) : <div className="identity-avatar avatar-fallback">{gameName.slice(0, 1)}</div>}<div>{gameProfile ? <a className="identity-name text-link" href={gameProfile} target="_blank" rel="noreferrer">{gameName}</a> : <strong className="identity-name">{gameName}</strong>}{item.game_identity_type ? <span>{formatConnectionType(item.game_identity_type)}</span> : null}<span>Discord: @{item.username}</span><div><span className="badge">{item.status.replaceAll("_", " ")}</span>{item.checked_in_at ? <span className="badge">Checked in</span> : null}</div></div></article>; })}</div> : <div className="empty-state">No participants have signed up yet.</div>}</section> : null}
