...
export default async function EventPage({ params }: { params: Promise<{ eventId: string }> }) {
  ...
  const canOperateMatches = isPrimaryHost || eventPermissions.includes("MANAGE_BRACKETS") || eventPermissions.includes("MANAGE_PARTICIPANTS") || ["FULL", "BRACKET", "SCOREKEEPER"].includes(cohostLevel);
  ...
  return (
    <div className="section-stack">
      ...
      {event.bracket_enabled ? <section className="panel section-stack"><div className="section-header"><div><h2>Tournament bracket</h2><p className="muted">{event.bracket_format?.replaceAll("_", " ").toLowerCase()} · {event.bracket_seeding_mode?.toLowerCase()} placement{event.bracket_require_check_in ? " · checked-in participants only" : ""}</p></div>{bracket ? <span className="badge">{bracket.status}</span> : null}</div><div className="workspace-card"><span className="card-kicker">{event.bracket_auto_generate ? "Automatic after signups" : "Host controlled"}</span><h3>{event.bracket_format === "THREE_PLAYER" ? "Three-player advancement" : "Single elimination"}</h3><p>Automatic byes, shared results, live Match Center operations, spectator viewing, and PNG export.</p></div><div className="button-row">{bracket?.settings_json && (canManageBracket || ["LIVE", "COMPLETED"].includes(bracket.status)) ? <Link className="button button-secondary" href={`/dashboard/events/${eventId}/bracket`}>View bracket</Link> : null}{bracket && (canOperateMatches || ["LIVE", "COMPLETED"].includes(bracket.status)) ? <Link className="button button-secondary" href={`/dashboard/events/${eventId}/matches`}>Open Match Center</Link> : null}{canManageBracket ? <Link className="button" href={`/dashboard/tools/bracket?eventId=${eventId}`}>Open bracket manager</Link> : null}</div></section> : null}
