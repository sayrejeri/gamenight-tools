...
type MatchRow = RowDataPacket & {
  id: string; round_number: number; match_number: number; status: string; scheduled_at: Date | null; no_show_deadline_at: Date | null;
  best_of: number; ready_a_at: Date | null; ready_b_at: Date | null; winner_entry_id: string | null;
  a_entry_id: string | null; a_user_id: string | null; a_name: string | null;
  b_entry_id: string | null; b_user_id: string | null; b_name: string | null;
...
export default async function EventMatchesPage({ params }: { params: Promise<{ eventId: string }> }) {
  ...
  const events = await query<EventRow[]>(
    `SELECT e.id, e.workspace_id, w.name AS workspace_name, e.name, CAST(e.primary_host_id AS CHAR) AS primary_host_id,
            e.status, e.visibility, e.bracket_enabled,
  ...
  const [workspaceAccess, cohostRows, participantRows, tournamentAccess, bracketRows] = await Promise.all([
  ...
  if (!bracket) {
    return <section className="panel section-stack"><h1>Match Center unavailable</h1><p className="muted">Generate and save the event bracket first.</p><Link className="button button-secondary" href={`/dashboard/events/${eventId}`}>Back to event</Link></section>;
  }
  if (!tournamentAccess.manager && !["LIVE", "COMPLETED"].includes(bracket.status)) {
    return <section className="panel section-stack"><h1>Match Center not live yet</h1><p className="muted">The host is still preparing the tournament.</p><Link className="button button-secondary" href={`/dashboard/events/${eventId}`}>Back to event</Link></section>;
  }
  ...
  const [settingsRows, matchRows, careerRows, championRows] = await Promise.all([
    ...
    query<MatchRow[]>(
      `SELECT bm.id, bm.round_number, bm.match_number, bm.status, bm.scheduled_at, bm.no_show_deadline_at,
              bm.best_of, bm.ready_a_at, bm.ready_b_at, bm.winner_entry_id,
              a.id AS a_entry_id, CAST(a.user_id AS CHAR) AS a_user_id, a.display_name AS a_name,
              b.id AS b_entry_id, CAST(b.user_id AS CHAR) AS b_user_id, b.display_name AS b_name,
       ...
       LEFT JOIN match_reports mr ON mr.id = (
         SELECT mr2.id FROM match_reports mr2 WHERE mr2.match_id = bm.id AND mr2.status <> 'VOID'
         ORDER BY mr2.created_at DESC LIMIT 1
       )
       LEFT JOIN match_disputes md ON md.id = (
         SELECT md2.id FROM match_disputes md2 WHERE md2.match_id = bm.id AND md2.status = 'OPEN'
         ORDER BY md2.created_at DESC LIMIT 1
       )
    ...
    query<ChampionRow[]>(
      `SELECT COUNT(*) AS total FROM bracket_entries be
  ...
  for (const match of matchRows) {
    ...
    for (const side of [{ id: match.a_entry_id, name: match.a_name ?? "Player A" }, { id: match.b_entry_id, name: match.b_name ?? "Player B" }]) {
  ...
  return (
    <div className="section-stack tournament-operations-page">
      <section className="page-heading"><div><span className="eyebrow">Tournament operations</span><h1>{event.name} Match Center</h1><p>Schedule matches, check in players, confirm results, handle disputes, and follow the live tournament.</p></div><div className="button-row"><Link className="button button-secondary" href={`/dashboard/events/${eventId}`}>Back to event</Link><Link className="button button-secondary" href={`/dashboard/events/${eventId}/bracket`}>View bracket</Link></div></section>

      <div className="tournament-stat-grid"><div className="stat-card"><span>Career record</span><strong>{wins}–{losses}</strong></div><div className="stat-card"><span>Current streak</span><strong>{streakType ? `${streakType}${streakCount}` : "—"}</strong></div><div className="stat-card"><span>Championships</span><strong>{championRows[0]?.total ?? 0}</strong></div><div className="stat-card"><span>Bracket stage</span><strong>{bracket.status.replaceAll("_", " ")}</strong></div></div>
      ...
      <div className="dashboard-grid">
        <section className="panel section-stack"><div><h2>Event standings</h2><p className="muted">Confirmed matches and staff-decided forfeits. Automatic byes do not count as wins.</p></div>{orderedStandings.length ? <div className="standings-list">{orderedStandings.map((record, index) => <div className="standing-row" key={record.name}><span>#{index + 1}</span><strong>{record.name}</strong><span>{record.wins} W · {record.losses} L</span></div>)}</div> : <div className="empty-state">No completed head-to-head matches yet.</div>}</section>
        <section className="panel section-stack"><div><h2>Your head-to-head</h2><p className="muted">Your confirmed record against opponents in this event.</p></div>{headToHead.size ? <div className="standings-list">{[...headToHead.values()].sort((a,b) => (b.wins+b.losses)-(a.wins+a.losses)).map((record) => <div className="standing-row" key={record.name}><strong>{record.name}</strong><span>{record.wins}–{record.losses}</span></div>)}</div> : <div className="empty-state">No head-to-head history in this event yet.</div>}</section>
      </div>
    </div>
  );
}
