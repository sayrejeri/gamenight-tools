...
type ParticipantRow = RowDataPacket & { user_id: string; status: string; checked_in_at: Date | null; joined_at: Date; game_identity_type: string | null; game_identity_value: string | null; discord_id: string; username: string; global_name: string | null; avatar_hash: string | null; connection_external_id: string | null; connection_profile_url: string | null; connection_avatar_url: string | null };
...
export default async function ParticipantsPage({ params }: { params: Promise<{ eventId: string }> }) {
  ...
  const participants = await query<ParticipantRow[]>(
    `SELECT ep.user_id, ep.status, ep.checked_in_at, ep.joined_at, ep.game_identity_type, ep.game_identity_value,
  ...
  return (
    <div className="section-stack">
      <section className="page-heading"><div><span className="eyebrow">Event signups</span><h1>Manage {event.name}</h1><p>Approve the final eligible list before generating or manually placing the bracket.</p></div><Link className="button button-secondary" href={`/dashboard/events/${eventId}`}>Back to event</Link></section>
      <section className="panel section-stack">
        ...
        {participants.length ? <div className="participant-management-list">{participants.map((item) => {
          ...
          return <article className="participant-management-row" key={item.user_id}><div className="identity-card">{avatar ? (profileUrl ? <a href={profileUrl} target="_blank" rel="noreferrer"><img className="identity-avatar" src={avatar} alt="" /></a> : <img className="identity-avatar" src={avatar} alt="" />) : <div className="identity-avatar avatar-fallback">{gameName.slice(0, 1)}</div>}<div>{profileUrl ? <a className="identity-name text-link" href={profileUrl} target="_blank" rel="noreferrer">{gameName}</a> : <strong className="identity-name">{gameName}</strong>}{item.game_identity_type ? <span>{formatConnectionType(item.game_identity_type)}</span> : null}<span>Discord: @{item.username}</span><span>Joined {new Date(item.joined_at).toLocaleString()}</span>{item.checked_in_at ? <span className="badge">Checked in</span> : null}</div></div><ParticipantStatusControl eventId={eventId} userId={item.user_id} initialStatus={item.status} /></article>;
        })}</div> : <div className="empty-state">No signup records yet.</div>}
      </section>
    </div>
  );
}
