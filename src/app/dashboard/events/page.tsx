...
type EventRow = RowDataPacket & { id: string; name: string; status: string; visibility: string; starts_at: Date | null; timezone: string; workspace_name: string; subgame_name: string | null; game_name: string | null; platform_name: string | null; game_thumbnail_url: string | null; participant_count: number; relationship: string };

export default async function EventsPage() {
  const session = await requireSession();
  const events = await query<EventRow[]>(
    `SELECT DISTINCT e.id, e.name, e.status, e.visibility, e.starts_at, e.timezone,
            ...
            CASE WHEN e.primary_host_id = ? THEN 'HOSTING'
                 ...
                 WHEN wm.user_id IS NOT NULL THEN 'SERVER ACCESS'
                 ELSE 'AVAILABLE' END AS relationship
     ...
     WHERE e.primary_host_id = ? OR ec.invited_user_id IS NOT NULL OR ep.user_id IS NOT NULL OR wm.user_id IS NOT NULL
        OR (e.status NOT IN ('DRAFT', 'AWAITING_APPROVAL') AND e.visibility = 'PUBLIC')
        OR (e.status NOT IN ('DRAFT', 'AWAITING_APPROVAL') AND e.visibility = 'SERVER' AND ug.user_id IS NOT NULL)
