...
import { canReviewProfiles, getPlatformRole } from "@/lib/platform-access";
...
export async function PATCH(request: NextRequest, context: { params: Promise<{ requestId: string }> }) {
  ...
  if (!canReviewProfiles(platformRole)) return NextResponse.json({ error: "Profile reviewer access is required." }, { status: 403 });
  const parsed = reviewSchema.safeParse(await request.json().catch(() => null));
  if (!parsed.success) return NextResponse.json({ error: "Invalid review decision." }, { status: 400 });
  const { requestId } = await context.params;

  const rows = await query<RequestRow[]>(
    `SELECT pr.*, u.discord_id AS applicant_discord_id, up.timezone,
            ug.icon_hash AS guild_icon_hash
     FROM profile_requests pr
     INNER JOIN users u ON u.id = pr.applicant_user_id
  ...
  if (!["PENDING", "CHANGES_REQUESTED"].includes(profileRequest.status)) {
  ...
  await withTransaction(async (connection) => {
        ...
        await connection.execute(
          `INSERT INTO teams
            (id, name, slug, tag, description, logo_url, banner_url, main_platform, main_game,
             region, recruiting_status, profile_status, verification_level, home_workspace_id,
             owner_user_id, reviewed_by, reviewed_at)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'INVITE_ONLY', 'APPROVED', ?, ?, ?, ?, CURRENT_TIMESTAMP(3))`,
          [createdProfileId, profileRequest.requested_name, profileRequest.requested_slug, payload.teamTag ?? null,
            profileRequest.description, profileRequest.logo_url, profileRequest.banner_url,
            profileRequest.main_platform, profileRequest.main_game, payload.region ?? null,
            parsed.data.verificationLevel, profileRequest.home_workspace_id,
            profileRequest.applicant_user_id, session.userId],
        );
        await connection.execute(
          `INSERT INTO team_members (team_id, user_id, role, status, invited_by, joined_at)
           VALUES (?, ?, 'OWNER', 'ACTIVE', ?, CURRENT_TIMESTAMP(3))`,
          [createdProfileId, profileRequest.applicant_user_id, session.userId],
        );
      } else {
        if (!profileRequest.discord_guild_id) throw new Error("Approved server request is missing its Discord server ID.");
        const iconUrl = profileRequest.logo_url || (profileRequest.guild_icon_hash
          ? `https://cdn.discordapp.com/icons/${profileRequest.discord_guild_id}/${profileRequest.guild_icon_hash}.png?size=512`
          : null);
        await connection.execute(
          `INSERT INTO workspaces
            (id, discord_guild_id, name, icon_url, banner_url, description, discord_invite_url,
             main_game_category, roblox_community_url, timezone, profile_status,
             verification_level, created_by)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'APPROVED', ?, ?)`,
          [createdProfileId, profileRequest.discord_guild_id, profileRequest.requested_name,
            iconUrl, profileRequest.banner_url, profileRequest.description, profileRequest.discord_invite_url,
            profileRequest.main_platform, profileRequest.roblox_community_url,
            profileRequest.timezone || "America/Detroit", parsed.data.verificationLevel,
            profileRequest.applicant_user_id],
        );
        await connection.execute(
          `INSERT INTO workspace_owner_claims (workspace_id, discord_id, created_by)
           VALUES (?, ?, ?)`,
          [createdProfileId, profileRequest.applicant_discord_id, session.userId],
        );
        await connection.execute(
    ...
    await connection.execute(
      `UPDATE profile_requests SET status = ?, reviewer_user_id = ?, review_reason = ?,
         reviewed_at = CURRENT_TIMESTAMP(3), created_profile_id = ?, updated_at = CURRENT_TIMESTAMP(3)
       WHERE id = ?`,
  ...
  await writeAuditLog({
    ...
    action: `profile_request.${parsed.data.decision.toLowerCase()}`,
    workspaceId: profileRequest.request_type === "SERVER" ? createdProfileId : null,
    targetType: profileRequest.request_type.toLowerCase(),
    targetId: createdProfileId ?? requestId,
    details: { requestId, reason: parsed.data.reason, verificationLevel: parsed.data.verificationLevel },
  });
