...
const bodySchema = z.object({
  status: z.enum(["PENDING", "APPROVED", "WAITLISTED", "REJECTED", "NO_SHOW", "DISQUALIFIED"]),
});
...
export async function PATCH(
  ...
  if (!allowed) return NextResponse.json({ error: "You cannot manage participants for this event." }, { status: 403 });

  const [result] = await getPool().execute(
    `UPDATE event_participants
     SET status = ?, checked_in_at = IF(? IN ('REJECTED', 'NO_SHOW', 'DISQUALIFIED'), NULL, checked_in_at)
     WHERE event_id = ? AND user_id = ?`,
    [parsed.data.status, parsed.data.status, eventId, userId],
  );
  ...
  await writeAuditLog({
    ...
    eventId,
    action: "event.participant.status_changed",
    targetType: "user",
    targetId: userId,
