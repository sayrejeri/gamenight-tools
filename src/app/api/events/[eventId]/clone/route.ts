...
const cloneSchema = z.object({ name: z.string().trim().min(2).max(160), keepSchedule: z.boolean().default(false) });
...
export async function POST(request: NextRequest, context: { params: Promise<{ eventId: string }> }) {
  ...
  const rows = await query<EventRow[]>(
    `SELECT id, workspace_id, CAST(primary_host_id AS CHAR) AS primary_host_id, description, game_name, platform_name, subgame_name,
            ...
            max_participants, timezone, rules_json, staff_approval_required, bracket_enabled, bracket_format, bracket_entry_mode,
            bracket_seeding_mode, bracket_auto_generate, bracket_require_check_in, bracket_group_count,
            bracket_advancers_per_group, bracket_tiebreak_mode
  ...
  const cohostRows = await query<(RowDataPacket & { permission_level: string })[]>(`SELECT permission_level FROM event_cohosts WHERE event_id = ? AND invited_user_id = ? AND status = 'ACCEPTED' LIMIT 1`, [eventId, session.userId]);
  ...
  await withTransaction(async (connection) => {
    await connection.execute(
      `INSERT INTO events
        (id, workspace_id, name, description, game_name, platform_name, subgame_name, game_url, game_external_id,
         ...
         bracket_enabled, bracket_format, bracket_entry_mode, bracket_seeding_mode, bracket_auto_generate, bracket_require_check_in,
      ...
      [
        ...
        source.bracket_entry_mode, source.bracket_seeding_mode, source.bracket_auto_generate, source.bracket_require_check_in,
        source.bracket_group_count, source.bracket_advancers_per_group, source.bracket_tiebreak_mode,
    ...
    if (source.bracket_enabled && source.bracket_format && source.bracket_seeding_mode) {
      await connection.execute(`INSERT INTO brackets (id, event_id, format, status, seeding_mode) VALUES (?, ?, ?, 'DRAFT', ?)`, [randomUUID(), newEventId, source.bracket_format, source.bracket_seeding_mode]);
    }
  });

  await writeAuditLog({ actorUserId: session.userId, workspaceId: source.workspace_id, eventId: newEventId, action: "event.cloned", targetType: "event", targetId: newEventId, details: { clonedFromEventId: eventId, keepSchedule: parsed.data.keepSchedule, bracketEntryMode: source.bracket_entry_mode, bracketFormat: source.bracket_format } });
