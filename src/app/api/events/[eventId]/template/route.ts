...
const bodySchema = z.object({ name: z.string().trim().min(2).max(160), description: z.string().trim().max(500).nullable().optional(), shared: z.boolean().default(true) });

type EventRow = RowDataPacket & {
  workspace_id: string; primary_host_id: string; name: string; description: string | null; platform_name: string | null;
...
export async function POST(request: NextRequest, context: { params: Promise<{ eventId: string }> }) {
  ...
  if (event.primary_host_id !== session.userId && !(await hasWorkspacePermission(session.userId, event.workspace_id, "MANAGE_EVENTS"))) {
    return NextResponse.json({ error: "Event-management permission is required to create this template." }, { status: 403 });
  }

  const configuration = {
    name: event.name, platformName: event.platform_name ?? "", subgameName: event.subgame_name ?? "", gameUrl: event.game_url ?? "",
    gameExternalId: event.game_external_id ?? "", gameUniverseId: event.game_universe_id ?? "", gameThumbnailUrl: event.game_thumbnail_url ?? "",
  ...
  await getPool().execute(
    `INSERT INTO event_templates
      (id, workspace_id, name, description, platform_name, subgame_name, game_url, game_external_id, game_universe_id,
       game_thumbnail_url, configuration_json, is_shared, created_by)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [id, event.workspace_id, parsed.data.name, parsed.data.description ?? null, event.platform_name, event.subgame_name,
     event.game_url, event.game_external_id, event.game_universe_id, event.game_thumbnail_url, JSON.stringify(configuration), parsed.data.shared ? 1 : 0, session.userId],
  );
