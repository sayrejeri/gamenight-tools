...
const saveSchema = z.object({ format: z.enum(["SINGLE_ELIMINATION", "THREE_PLAYER"]), seedingMode: z.enum(["RANDOM", "MANUAL"]), state: z.unknown() });
...
export async function GET(_request: NextRequest, context: { params: Promise<{ eventId: string }> }) {
  ...
  const rows = await query<(RowDataPacket & { format: string; seeding_mode: string; settings_json: string | null; status: string })[]>(
    `SELECT format, seeding_mode, settings_json, status FROM brackets WHERE event_id = ? LIMIT 1`, [eventId],
  );
  const bracket = rows[0];
  return NextResponse.json({ bracket: bracket ? { format: bracket.format, seedingMode: bracket.seeding_mode, status: bracket.status, state: bracket.settings_json ? JSON.parse(bracket.settings_json) : null } : null });
}
  ...
  await getPool().execute(
    `INSERT INTO brackets (id, event_id, format, status, seeding_mode, settings_json, generated_at)
     ...
     ON DUPLICATE KEY UPDATE format = VALUES(format), status = 'GENERATED', seeding_mode = VALUES(seeding_mode),
       settings_json = VALUES(settings_json), generated_at = CURRENT_TIMESTAMP(3), updated_at = CURRENT_TIMESTAMP(3)`,
    [bracketId, eventId, parsed.data.format, parsed.data.seedingMode, serialized],
  );
  await writeAuditLog({ actorUserId: session.userId, workspaceId: access.event.workspace_id, eventId, action: "bracket.saved", targetType: "bracket", targetId: eventId, details: { format: parsed.data.format, seedingMode: parsed.data.seedingMode } });
