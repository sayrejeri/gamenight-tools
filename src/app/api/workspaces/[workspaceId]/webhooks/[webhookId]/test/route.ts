...
export async function POST(_request: Request, context: { params: Promise<{ workspaceId: string; webhookId: string }> }) {
  ...
  if (role !== "OWNER" && role !== "ADMIN") return NextResponse.json({ error: "Server owner or admin access is required." }, { status: 403 });
  ...
  try {
    const status = await sendDiscordWebhook(decryptWebhookUrl(webhook.encrypted_url), {
      ...
      embed: { title: "Webhook connected", description: `${webhook.label} is ready to post updates for ${webhook.workspace_name}.`, fields: [{ name: "Source", value: "Game Night Tools v0.3", inline: true }, { name: "Status", value: "Test successful", inline: true }] },
    });
    await getPool().execute(
      `UPDATE workspace_webhooks SET failure_count = 0, last_success_at = CURRENT_TIMESTAMP(3), last_error_message = NULL WHERE id = ?`, [webhookId]);
    await getPool().execute(
      `INSERT INTO webhook_delivery_logs (id, webhook_id, notification_type, status, response_status)
       VALUES (?, ?, 'TEST', 'SUCCESS', ?)`, [randomUUID(), webhookId, status]);
    return NextResponse.json({ success: true });
  } catch (error) {
    const message = error instanceof Error ? error.message.slice(0, 500) : "Webhook test failed.";
    await getPool().execute(
      `UPDATE workspace_webhooks SET failure_count = failure_count + 1, last_error_at = CURRENT_TIMESTAMP(3), last_error_message = ? WHERE id = ?`, [message, webhookId]);
    await getPool().execute(
      `INSERT INTO webhook_delivery_logs (id, webhook_id, notification_type, status, error_message)
       VALUES (?, ?, 'TEST', 'FAILED', ?)`, [randomUUID(), webhookId, message]);
