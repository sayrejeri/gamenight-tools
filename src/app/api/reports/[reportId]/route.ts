...
export async function PATCH(request: NextRequest, context: { params: Promise<{ reportId: string }> }) {
  ...
  if (!canModeratePlatform(role)) return NextResponse.json({ error: "Platform moderator access is required." }, { status: 403 });
  ...
  await getPool().execute(
    `UPDATE reports SET status = ?, assigned_to = COALESCE(assigned_to, ?),
  ...
  await writeAuditLog({ actorUserId: session.userId, action: `report.${parsed.data.status.toLowerCase()}`, targetType: "report", targetId: reportId, details: { note: parsed.data.resolutionNote } });
